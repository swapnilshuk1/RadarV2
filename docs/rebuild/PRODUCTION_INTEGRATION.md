...
6. `scripts/complete-staged-rollout.ts --context=<context> --expected-active=<previous context> --exclude-source-version=<known navigation source> --activate` supervises dependency release, newly ready scheduling, dossier composition and publication. Run one supervisor alongside the evaluation and enrichment workers. It waits for queued evaluation/enrichment work, skips retained failed presentations, and stops with a blocker if unresolved work remains. It changes the active pointer only in a transaction that verifies both the expected old pointer and complete prepared coverage of current eligible versions. Failed presentations must be investigated and explicitly retried before restarting the supervisor. Without `--activate`, it prepares coverage only.

Do not put `--watch` directly on a PM2 start command: PM2 can interpret it as filesystem watching. Supply worker arguments through a process configuration with `watch: false`. The worker's `--watch` option waits for delayed evaluation retries; it does not watch files.
...
The 16 verified eligible navigation captures have explicit source-recovery obligations in the existing recovery queue (`SOURCE_NOT_JOB_DESCRIPTION` / `WRONG_PAGE`). The feed shows them as needing attention. The activation coverage check accounts for these exact versions as excluded, retains their original source and evaluation history, and does not call them successfully evaluated. A new valid acquired version must earn its own evaluation and dossier.
