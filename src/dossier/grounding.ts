...
export function validatePassages(composition: unknown, research: Research): void {
  ...
  const hasHardScreenLineage = (id: string, visited = new Set<string>()): boolean => {
    if (hardScreenRoleClaims.has(id)) return true;
    if (visited.has(id)) return false;
    visited.add(id);
    return claims.get(id)?.derivedFrom.some(parentId => hasHardScreenLineage(parentId, visited)) ?? false;
  };
  const resolveId = (id: string) => {
    if (claims.has(id)) return id;
    const alt1 = id.replace(/-(?:claim-)?(\d+)$/, '-$1');
