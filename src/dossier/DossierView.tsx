    ...
    <div className="dossier-body">
      ...
      {chapter('IX','Supporting evidence','Evidence behind this recommendation',<>
        ...
        <details className="dossier-appendix"><summary>Evidence, context acquisition & claim lineage</summary>
          <h3>Decision requirements</h3>{d.verdict.requirements.map((r,i) => <div className="dossier-ledger-row" key={i}><strong>{r.requirement}</strong><span>{r.mandatory ? 'Required' : 'Preferred'} · {r.status.replaceAll('_',' ').toLowerCase()}</span><p>{r.reasoning}</p></div>)}
