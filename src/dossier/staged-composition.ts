...
import { composeDossier, memoSectionPurpose } from "./composition";
...
const editorialSchema = z
  ...
  .strict();
const productionEditorialSchema = editorialSchema.extend({
  ...
  .object({
    reviews: z.array(
      z
        .object({
          passageId: z.string(),
          externalComparison: z.enum(["NONE", "SOURCE_SUPPORTED", "UNSUPPORTED"]),
          candidateAbsence: z.enum(["NONE", "SOURCE_SCOPED", "UNSUPPORTED"]),
          factualAssessment: z.string().min(1),
          supported: z.boolean(),
          issue: z.string(),
        })
        .strict(),
...
export async function reviewStagedEditorialFacts(
  model: ReasoningModel,
  frozen: StagedResearchInput,
  section: string,
  value: unknown,
  memo?: {
    points: z.infer<typeof memoPointSchema>[];
    prior: unknown;
    renderedScope?: FieldResolution[];
  },
): Promise<FactualReviewReceipt> {
  const passages = reviewPassages(value);
  const receipt = (): FactualReviewReceipt => ({
    section,
    contentFingerprint: reviewFingerprint(value),
    ...
    reviewer: `${model.id}/${model.version}`,
    policyVersion: FACTUAL_REVIEW_POLICY_VERSION,
    passageIds: passages.map((p) => p.passageId),
    accepted: true,
    ...(memo
      ? {
          coveredPointIds: memo.points.map((p) => p.id),
          planFingerprint: reviewFingerprint(memo.points),
        }
      : {}),
  });
  ...
  const include = (id: string) => {
    ...
    const claim = known.get(id);
    if (!claim) throw new Error("EDITORIAL_FACT_REFERENCE_UNKNOWN");
    cited.add(id);
    claim.derivedFrom.forEach(include);
  };
...
Grounded inference is allowed and required. Accept labelled interpretations, conditional advice, honest ranges/topology and decision hinges grounded in the evidence. Do not demand literal quotations for an inference or delete useful sections. Exact numbers, named relationships and external factual premises still need support. Distinguish a conditional question from an asserted fact; do not reject a company uncertainty as if it were a candidate deficit. Do not change or reconsider pursuit verdict, screening viability, requirement mapping or career-capital adjudication. Those decisions are outside this factual review.`;
  const responseSchema = memo
    ? factualReviewSchema
        .extend({ coveredPointIds: z.array(z.string()), editorialIssues: z.array(z.string()) })
        .strict()
    : factualReviewSchema;
  const editorialInstruction = memo
    ? ` Also review editorial coverage. Return coveredPointIds for assigned points whose distinct meaning survives in this section, and editorialIssues for lost qualifications, semantic repetition of prior prose (brief thesis previews and genuinely distinct actions are allowed), or an unreadable long bullet. Judge principles and meaning, never word-for-word similarity. Do not require every cited fact to be repeated or infer candidate absence from unmentioned claims. A point about missing information is covered by a useful question and consequence. Supplied renderedScope is also visible in this section as an application-owned table: points established there are covered without repeated prose. The writing plan is an editorial aid, not evidence: never require an unsupported premise from it. A point is covered when its material meaning is accurately addressed with necessary qualifications, including correcting an overstatement. Do not require candidate-fit detail inside a role-side mandate point. Do not reevaluate the fixed decision.`
    : "";
  const input = {
    memo: memo
      ? {
          assignedPoints: memo.points,
          renderedScope: memo.renderedScope,
          priorPassages: allPassages(memo.prior).map((p) => p.text),
        }
      : undefined,
    factualReview: {
      section,
      audience: "Personalized advice to the candidate under assessment",
      passages,
      claims,
      candidateConflicts: frozen.candidateConflicts,
      sources: frozen.sources
        .filter((s) => sourceIds.has(s.id))
        .map(({ id, plane, title, attribution, locator }) => ({
          id,
          plane,
          title,
          attribution,
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const response = await model.generate(
      ...
      model.schemaFormat === "json-schema" || /bedrock/i.test(model.id)
        ? bedrockJsonSchema(responseSchema)
        : modelSchema(responseSchema),
    );
    try {
      const result = responseSchema.parse(response);
      if (memo) {
        const checked = result as z.infer<typeof factualReviewSchema> & {
          coveredPointIds: string[];
          editorialIssues: string[];
        };
        const expectedIds = new Set(memo.points.map((p) => p.id));
        if (checked.coveredPointIds.some((id) => !expectedIds.has(id)))
          throw new Error("EDITORIAL_COVERAGE_IDENTITY_INVALID");
        const missing = [...expectedIds].filter((id) => !checked.coveredPointIds.includes(id));
        if (missing.length || checked.editorialIssues.length)
          throw new Error(
            `MEMO_EDITORIAL_REPAIR: missing points ${missing.join(", ")}; ${checked.editorialIssues.join("; ")}`,
          );
      }
      const parsed = result.reviews;
      const expected = new Set(passages.map((p) => p.passageId));
      const seen = new Set<string>();
      for (const review of parsed) {
        if (!expected.has(review.passageId) || seen.has(review.passageId))
          throw new Error("EDITORIAL_FACT_REVIEW_IDENTITY_INVALID");
        seen.add(review.passageId);
        if (review.supported === Boolean(review.issue.trim()))
          throw new Error("EDITORIAL_FACT_REVIEW_RESULT_INVALID");
      }
      if (seen.size !== expected.size) throw new Error("EDITORIAL_FACT_REVIEW_INCOMPLETE");
      reviews = parsed;
      break;
    } catch (error) {
      if (error instanceof Error && error.message.startsWith("MEMO_EDITORIAL_REPAIR:")) throw error;
      await model.discardResponse?.(response);
      repair = error instanceof Error ? error.message : "Invalid review response";
    }
  }
  // A broken reviewer response is not evidence that the prose needs rewriting.
  if (!reviews) throw new ModelProviderUnavailableError(`EDITORIAL_FACT_REVIEW_INVALID: ${repair}`,undefined,30_000);
  const defects = reviews.filter(
    (r) =>
      !r.supported ||
      r.externalComparison === "UNSUPPORTED" ||
      r.candidateAbsence === "UNSUPPORTED",
  );
  if (defects.length)
    throw new Error(
      `EDITORIAL_FACT_SUPPORT: ${defects.map((r) => `${r.passageId}: ${r.issue || r.factualAssessment}`).join("; ")}`,
    );
  return receipt();
}
export async function reviewStagedEditorialAction(
  model: ReasoningModel,
  verdict: StagedDecisionResult["decision"]["verdict"],
  section: string,
  value: unknown,
) {
  if (allPassages(value).some((p) => /\b(?:PURSUE|CONSIDER|PASS)\b/.test(p.text)))
    throw new Error(
      "Action labels belong in the application-rendered verdict, not narrative prose. Describe the next step in plain language without uppercase action codes.",
    );
  const action =
    verdict === "PASS"
      ? "DO_NOT_PURSUE"
...
Return only rationale and narrativePlan. narrativePlan.memoPoints must assign every material point to exactly one of executiveThesis, opportunityValue, mandate, candidateFit, decisionConditions, approach. Each point has a unique id, section, concise point, valid claimIds, requirementIds and resolutionFields (empty arrays when not applicable). Cover every requirement ID in candidateFit points and every decision hinge in decisionConditions. Cover material career-capital dimensions in opportunityValue or decisionConditions according to whether they establish value or require resolution. Follow sectionResponsibilities: mandate contains role-side delivery and scope only; candidate achievements and fit belong in candidateFit. Do not combine points belonging to different sections. Include distinct company context, identity alignment, authority, success outcomes and useful next actions. Source uncertainties must survive; do not merely assign topics. Scope facts rendered in the application scope table do not need duplicate prose; planned mandate points may reference them. The thesis previews the central argument; evidence is explained once in its primary section. No overlapping assignments of the same argument. Aim for 15-25 distinct points, adapted to complexity. Explain the fixed verdict richly and concretely. A PASS can still describe a valuable mandate and strong transferable capability.
...
export async function composeStagedDossier(
    ...
    try {
      previous = await model.generate(
        instruction,
        {
          ...
          sectionResponsibilities: memoSectionPurpose,
          staged,
          ...(issue
            ? {
                previous,
                repair: `Repair this defect while preserving the other valid points and references: ${issue}. Return the complete plan.`,
              }
            : {}),
        },
        bindMemoReferences(
          /bedrock/i.test(model.id)
            ? bedrockJsonSchema(productionEditorialSchema)
            : modelSchema(productionEditorialSchema),
          {
      ...
      productionEditorialSchema.parse(previous);
      const candidate = bindStagedEditorial(frozen, staged, previous);
      validateMemoPlan(candidate, staged);
      research = candidate;
      break;
    } catch (error) {
      if (error instanceof ModelProviderUnavailableError) throw error;
      issue = error instanceof Error ? error.message : "Invalid editorial plan";
    }
  }
  if (!research) {
    // Preserve the repair chain, but allow a later bounded attempt to replace
    // its exhausted tail instead of replaying the same invalid plan forever.
    await model.discardResponse?.(previous);
    throw new Error(`STAGED_EDITORIAL_FAILED: ${issue}`);
  }
  ...
  const dossier = await composeDossier(frozen, research, model, onStage, {
    decisionContext: {
      ...
      action:
        staged.decision.verdict === "PASS"
          ? "DO_NOT_PURSUE"
          : staged.decision.verdict === "PURSUE"
            ? "PURSUE"
            : "INVESTIGATE_BEFORE_COMMITTING",
      instruction:
        "PASS means do not pursue, never passes screening. Explain this fixed action, including limitations and reopening conditions. Role value and candidate strengths remain valuable to describe even for PASS. Do not silently substitute a different recommendation in prose.",
    },
    validateSection: async (section, value, prior) => {
      validateMemoSectionCoverage(section, value, research!.narrativePlan, staged);
      onStage(`Reviewing factual support: ${section}`);
      factualReviews.set(
        section,
        await reviewStagedEditorialFacts(factualReviewer, frozen, section, value, {
          points: research!.narrativePlan.memoPoints!.filter((p) => p.section === section),
          prior,
          renderedScope:
            section === "mandate"
              ? research!.resolutions.filter((r) =>
                  (scopeFields as readonly string[]).includes(r.field),
                )
              : undefined,
        }),
      );
      if (section !== "executiveThesis" && section !== "approach") return;
      const review = await reviewStagedEditorialAction(
        model,
  ...
  return {
    ...
    generation: {
      ...
      factualReviews: [...factualReviews.values()],
    },
