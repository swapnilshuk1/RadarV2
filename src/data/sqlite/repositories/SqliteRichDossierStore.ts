...
export class SqliteRichDossierStore {
  ...
  async get(identity:DossierPresentationIdentity,inputFingerprint:string):Promise<Dossier|null> {
    ...
    if(!row||row.source_evaluation_fingerprint!==inputFingerprint)return null;
    try {return dossierSchema.parse(JSON.parse(row.presentation_json));}catch{return null;}
  }
  async save(identity:DossierPresentationIdentity,inputFingerprint:string,dossier:Dossier):Promise<void> {
    ...
    await this.db.execute(`INSERT INTO materialized_dossier_presentations(tenant_id,person_id,canonical_job_id,opportunity_version,evaluation_context_fingerprint,presentation_version,source_evaluation_fingerprint,presentation_json,generated_at) VALUES(?,?,?,?,?,?,?,?,?) ON CONFLICT(tenant_id,person_id,canonical_job_id,opportunity_version,evaluation_context_fingerprint,presentation_version) DO NOTHING`,[identity.tenantId,identity.personId,identity.canonicalJobId,identity.opportunityVersion,identity.evaluationContextFingerprint,RICH_DOSSIER_VERSION,inputFingerprint,JSON.stringify(parsed),parsed.generatedAt]);
  }
}
