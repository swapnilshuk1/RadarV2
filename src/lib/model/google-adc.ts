...
export function adcTokenProvider() {
  ...
  return async () => {
    ...
    const explicitFile=process.env.GOOGLE_APPLICATION_CREDENTIALS;
    const locations = explicitFile?[explicitFile]:[
    ...
    for (const file of locations) {
      let credentials;
      try { credentials = JSON.parse(await readFile(file, 'utf8')); } catch { if(explicitFile)throw new Error('Explicit ADC credential file is unreadable or invalid');continue; }
      ...
      if (!token.access_token || !Number.isFinite(token.expires_in) || token.expires_in<=60) throw new Error('ADC returned an invalid token lifetime');
