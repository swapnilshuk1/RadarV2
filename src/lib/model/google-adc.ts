...
export function adcTokenProvider(auth: Pick<GoogleAuth, 'getAccessToken'> = new GoogleAuth({
  ...
  return () => {
    ...
    pending ??= new Promise<string | null | undefined>((resolve, reject) => {
      const timer = setTimeout(() => reject(new Error('Google ADC authentication timed out')), timeoutMs);
      auth.getAccessToken().then(resolve, reject).finally(() => clearTimeout(timer));
    }).then(token => {
