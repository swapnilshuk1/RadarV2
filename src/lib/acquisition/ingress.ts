...
} from "./CanonicalIngestionService";
...
export function parseAcquisitionEnvelope(value: unknown): AcquisitionEnvelope {
  if (!value || typeof value !== "object" || Array.isArray(value))
    throw new AcquisitionIngressError(400, "MALFORMED_ENVELOPE");
  const input = value as Record<string, unknown>;
  const text = (key: string, max = 512) => {
    const item = input[key];
    if (typeof item !== "string" || !item.trim() || item.length > max)
      throw new AcquisitionIngressError(400, `INVALID_${key.toUpperCase()}`);
    return item.trim();
  };
  if (input.schemaVersion !== ACQUISITION_ENVELOPE_VERSION)
    throw new AcquisitionIngressError(400, "UNSUPPORTED_ENVELOPE_VERSION");
  if (!input.payload || typeof input.payload !== "object" || Array.isArray(input.payload))
    throw new AcquisitionIngressError(400, "INVALID_PAYLOAD");
  ...
  for (const key of [
    ...
    if (typeof payload[key] !== "string" || !payload[key].trim())
      throw new AcquisitionIngressError(400, `INVALID_PAYLOAD_${key.toUpperCase()}`);
  }
  ...
  if (
    ...
    (payload.sourcePayloadKey.startsWith("/") ||
      payload.sourcePayloadKey.startsWith("~") ||
      /^[a-zA-Z]:[\\/]/.test(payload.sourcePayloadKey))
  ) {
  ...
  const envelope: AcquisitionEnvelope = {
    ...
    submissionId: text("submissionId", 160),
    runId: text("runId", 160),
    tenantId: text("tenantId", 160),
    personId: text("personId", 160),
    searchPlanId: input.searchPlanId == null ? null : text("searchPlanId", 160),
    contentHash: text("contentHash", 128),
    payload,
  };
  if (!/^[a-f0-9]{64}$/i.test(envelope.contentHash))
    throw new AcquisitionIngressError(400, "INVALID_CONTENT_HASH");
  const actual = computeContentHash({
    title: payload.jobTitle.trim(),
    companyName: payload.companyName?.trim() || null,
    location: payload.location || null,
    employmentType: payload.employmentType || null,
    rawContent: payload.rawContent.trim(),
  });
  if (!crypto.timingSafeEqual(Buffer.from(actual), Buffer.from(envelope.contentHash.toLowerCase())))
    throw new AcquisitionIngressError(422, "CONTENT_HASH_MISMATCH");
  return envelope;
}
...
export class AcquisitionIngressService {
  constructor(
    ...
    private readonly ingestion = new CanonicalIngestionService(db),
  ) {}

  async submit(envelope: AcquisitionEnvelope): Promise<CanonicalIngestionResult> {
    const prior = await this.db.one<{ content_hash: string; response_json: string }>(
      "SELECT content_hash, response_json FROM acquisition_ingress_submissions WHERE submission_id = ?",
      [envelope.submissionId],
    );
    if (prior) {
      if (prior.content_hash !== envelope.contentHash)
        throw new AcquisitionIngressError(409, "IDEMPOTENCY_KEY_PAYLOAD_CONFLICT");
    ...
    try {
      await this.db.execute(
        ...
        [
          ...
          result.opportunityVersion,
          JSON.stringify(result),
        ],
      );
      return result;
    } catch (error) {
      const winner = await this.db.one<{ content_hash: string; response_json: string }>(
        "SELECT content_hash, response_json FROM acquisition_ingress_submissions WHERE submission_id = ?",
        [envelope.submissionId],
      );
      if (winner?.content_hash === envelope.contentHash)
        return JSON.parse(winner.response_json) as CanonicalIngestionResult;
      throw error;
    }
  }
  ...
  private async ensureRemoteRun(envelope: AcquisitionEnvelope): Promise<void> {
    await this.db.execute(
      ...
      [
        ...
        JSON.stringify({
          acquisitionBoundary: "localhost_to_oracle",
  ...
  private async recordCanonicalLineage(
    ...
    await this.db.transaction(async (tx) => {
      await tx.execute(
        ...
        [
          ledgerId,
          result.canonicalJobId,
          envelope.payload.sourcePortal,
          envelope.payload.sourceJobId,
          envelope.payload.canonicalUrl,
          envelope.payload.jobTitle,
          envelope.payload.companyName || "Unknown",
          envelope.payload.location || null,
          now,
          now,
          now,
          now,
      ...
      await tx.execute(
        ...
        [
          ...
          envelope.personId,
          ledger.id,
          `ingress:${envelope.submissionId}`,
          envelope.payload.sourcePortal,
          envelope.payload.sourceJobId,
          envelope.payload.canonicalUrl,
          envelope.payload.finalUrl || null,
          result.contentHash,
          result.canonicalJobId,
          result.opportunityVersion,
        ],
...
export async function handleAcquisitionIngress(request: Request): Promise<Response> {
  ...
  if (!isValidAcquisitionIngressSecret(secret, supplied))
    return new Response(JSON.stringify({ error: "Unauthorized" }), {
      status: 401,
      headers: { "content-type": "application/json" },
    });
  const length = Number(request.headers.get("content-length") || 0);
  if (length > MAX_ACQUISITION_ENVELOPE_BYTES)
  ...
  try {
    ...
    if (Buffer.byteLength(body, "utf8") > MAX_ACQUISITION_ENVELOPE_BYTES)
      throw new AcquisitionIngressError(413, "PAYLOAD_TOO_LARGE");
    const result = await new AcquisitionIngressService().submit(
      parseAcquisitionEnvelope(JSON.parse(body)),
    );
    ...
    const known =
      error instanceof AcquisitionIngressError
        ? error
        : new AcquisitionIngressError(422, "ACQUISITION_REJECTED");
 ...
 */
export async function handleAcquisitionIngressRunTerminalization(
  request: Request,
): Promise<Response> {
  if (request.method !== "POST")
    return new Response(JSON.stringify({ error: "Method Not Allowed" }), {
  ...
  if (
    !isValidAcquisitionIngressSecret(
      process.env.RADAR_ACQUISITION_INGRESS_SECRET,
      request.headers.get("x-radar-acquisition-key"),
    )
  ) {
    return new Response(JSON.stringify({ error: "Unauthorized" }), {
      status: 401,
  ...
  try {
    ...
    if (status !== "enriching" && status !== "failed" && status !== "aborted")
      throw new AcquisitionIngressError(400, "INVALID_RUN_STATUS");
    const eligibleStatuses =
      status === "enriching"
        ? "'running'"
        : "'queued','initializing','running','waiting_for_confirmation','stopping','enriching','completing'";
    const changed = await getDatabaseAdapter().execute(
      `UPDATE scrape_runs SET status=?,
             finished_at=CASE WHEN ? IN ('failed','aborted') THEN CURRENT_TIMESTAMP ELSE finished_at END,
      ...
      [
        ...
        text("tenantId"),
        text("personId"),
        input.searchPlanId ?? null,
        input.searchPlanId ?? null,
      ],
    );
    return new Response(JSON.stringify({ terminalized: changed.rowsAffected > 0 }), {
      status: 200,
      headers: { "content-type": "application/json" },
    });
  } catch (error) {
    const known =
      error instanceof AcquisitionIngressError
        ? error
        : new AcquisitionIngressError(422, "RUN_TERMINALIZATION_REJECTED");
    return new Response(JSON.stringify({ error: known.message }), {
      status: known.status,
      headers: { "content-type": "application/json" },
    });
