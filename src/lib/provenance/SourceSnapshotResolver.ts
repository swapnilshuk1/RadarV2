...
export class SourceSnapshotResolver {
  ...
  async captureOpportunityVersionRef(
    ...
    this.assertOpportunityRowIdentity(row);
    ...
    const bytes = await this.loadBlob(row.source_payload_key);
    return {
      kind: "OPPORTUNITY_VERSION",
      canonicalJobId,
      opportunityVersion,
      contentHash: row.content_hash,
      sourcePayloadKey: row.source_payload_key,
      sourcePayloadSha256: sha256(bytes),
    };
  }
  ...
  private async resolveOpportunityVersion(
    ...
    this.assertOpportunityRowIdentity(row);
    if (row.content_hash !== ref.contentHash || row.source_payload_key !== ref.sourcePayloadKey) {
      throw new SourceSnapshotIdentityMismatchError(
    ...
    if (row.source_payload_key) {
      if (!ref.sourcePayloadSha256) {
        throw new SourceSnapshotIdentityMismatchError(
          `Opportunity source digest is missing for ${ref.canonicalJobId}/${ref.opportunityVersion}.`,
        );
      }
      const bytes = await this.loadBlob(row.source_payload_key);
      if (sha256(bytes) !== ref.sourcePayloadSha256) {
        throw new SourceSnapshotIdentityMismatchError(
    ...
    if (ref.sourcePayloadSha256 !== null) {
      throw new SourceSnapshotIdentityMismatchError(
        `Unexpected source payload digest for database-text opportunity ${ref.canonicalJobId}/${ref.opportunityVersion}.`,
      );
    }
  ...
  private assertOpportunityRowIdentity(row: OpportunitySourceRow): void {
    if (row.source_payload_key) return;
  ...
  private async loadBlob(payloadKey: string): Promise<Buffer> {
    if (!this.blobStore) {
      throw new SourceSnapshotUnavailableError(
        `BlobStore is required to resolve source payload ${payloadKey}.`,
      );
    }
    const bytes = await this.blobStore.get(payloadKey);
    if (!bytes) {
      throw new SourceSnapshotUnavailableError(
        `Source payload is unavailable in BlobStore: ${payloadKey}`,
      );
    }
    return bytes;
  }
  ...
  private async loadOpportunitySource(
    ...
    const row = await this.db.one<OpportunitySourceRow>(
      `SELECT id, canonical_job_id, content_hash, job_title, company_name, location,
