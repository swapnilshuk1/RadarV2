...
export interface ResolvedScraperPlan {
  ...
  readonly criteria: import("../domain/evaluation_context").SearchCriteriaPayload;
...
export interface ScraperAuthResolution {
  ...
  readonly resolvedPlan?: ResolvedScraperPlan;
}
...
export async function resolveScraperAuthContext(
  ...
  const resolvedPlan = await resolveActiveScraperPlan(
    resolved.scope,
    resolved.activeContext,
    db,
    searchPlanIdOverride
  );
...
/**
 * Authoritative Scraper Plan Resolver:
 * Resolves the active search plan, criteria, and compiles discrete ranked queries
 * via SearchPlanner without leaking domain criteria transformation into scraper execution.
 */
export async function resolveActiveScraperPlan(
  ...
  // 2. Fallback to candidate state in Turso if no active plan exists
  if (!criteria) {
    try {
