    ...
    try {
      if (metrics.tenantId && metrics.searchPlanId && metrics.evaluationContextFingerprint) {
        ...
        const checks: MetricIntegrityCheck[] = [
          ...
          { code: "CHECK_STATE_EVALUATED", metricName: "evaluationPopulation.evaluated", expected: state.evaluated, actual: metrics.evaluationPopulation.evaluated, status: state.evaluated === metrics.evaluationPopulation.evaluated ? "PASS" : "ERROR", message: "Independently reconciled evaluated bucket." },
          { code: "CHECK_STATE_SPARSE", metricName: "evaluationPopulation.sparse", expected: state.sparse, actual: metrics.evaluationPopulation.sparse, status: state.sparse === metrics.evaluationPopulation.sparse ? "PASS" : "ERROR", message: "Independently reconciled sparse bucket." },
          ...
          { code: "CHECK_ENGINE_CONSIDER", metricName: "engineBreakdown.consider", expected: state.consider, actual: metrics.engineBreakdown.consider, status: state.consider === metrics.engineBreakdown.consider ? "PASS" : "ERROR", message: "Independently reconciled engine CONSIDER bucket." },
          { code: "CHECK_ENGINE_PASS", metricName: "engineBreakdown.pass", expected: state.pass, actual: metrics.engineBreakdown.pass, status: state.pass === metrics.engineBreakdown.pass ? "PASS" : "ERROR", message: "Independently reconciled engine PASS bucket." },
