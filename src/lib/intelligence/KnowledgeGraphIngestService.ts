...
export class KnowledgeGraphIngestService {
   ...
   */
  public ingest(graph: KnowledgeGraph, initialReport: KnowledgeGraphBuildReport): KnowledgeGraphBuildReport {
    const report = { ...initialReport };

    // 1. Source
    // Source idempotency is handled by the repository ON CONFLICT DO UPDATE
    this.repos.sources.recordSource(graph.source);

    // 2. Company
    // We check if it already exists to report on it
    const existingCompany = this.repos.companies.findByName(graph.company.name);
    if (existingCompany) {
      ...
      this.repos.companies.registerCompany(graph.company);
    }

    // 3. Opportunity
    const existingOps = this.repos.opportunities.findOpportunities({ companyId: graph.company.id });
    ...
    this.repos.opportunities.mergeOpportunity(graph.opportunity);
    ...
    this.repos.acquisition.recordDocument(graph.document);
    ...
    const existingFacts = this.repos.knowledge.findFactsForOpportunity(graph.opportunity.id);
    ...
    if (graph.evidence.length > 0) {
      this.repos.knowledge.recordEvidence(graph.evidence);
    }
    
    if (factsToInsert.length > 0) {
      this.repos.knowledge.recordFacts(factsToInsert);
    }
