...
import { isStagedPolicy } from './evaluationQueuePolicy';
...
export class EvaluationWorkScheduler {
  ...
  async ensureWork(input:EvaluationWorkIdentity):Promise<{jobId:string;queued:boolean;requirementStatus:string}>{
    ...
    const staged=isStagedPolicy(context.policy_version);
    ...
    const effectiveStatus=await this.db.transaction(async tx=>{
      ...
      const requirement=await tx.one<{status:string}>(`SELECT status FROM evaluation_requirements WHERE tenant_id=? AND person_id=? AND search_plan_id=? AND canonical_job_id=? AND opportunity_version=? AND evaluation_context_fingerprint=?`,[input.tenantId,input.personId,input.searchPlanId,input.canonicalJobId,input.opportunityVersion,input.evaluationContextFingerprint]);
      // Terminal requirements must never be resurrected by a repeated inventory pass.
