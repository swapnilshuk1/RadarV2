...
export class EvaluationWorkScheduler {
  ...
  async ensureWork(input:EvaluationWorkIdentity):Promise<{jobId:string;queued:boolean;requirementStatus:string}>{
    ...
    const ready=enrichment?.status==='COMPLETE'; const requirementStatus=ready?'READY':'WAITING_ENRICHMENT'; const jobStatus=ready?'pending':'waiting_enrichment'; const suffix=createHash('sha256').update(input.evaluationContextFingerprint).digest('hex').slice(0,12);
    ...
    await this.db.transaction(async tx=>{
      ...
      await tx.execute(`INSERT INTO evaluation_jobs (id,tenant_id,person_id,search_plan_id,canonical_job_id,opportunity_version,evaluation_context_fingerprint,status,attempts,max_attempts,next_attempt_at) VALUES (?,?,?,?,?,?,?, ?,0,3,CURRENT_TIMESTAMP) ON CONFLICT(tenant_id,search_plan_id,canonical_job_id,opportunity_version,evaluation_context_fingerprint) DO UPDATE SET status='pending' WHERE evaluation_jobs.status='waiting_enrichment' AND ?='pending'`,[jobId,input.tenantId,input.personId,input.searchPlanId,input.canonicalJobId,input.opportunityVersion,input.evaluationContextFingerprint,jobStatus,jobStatus]);
    });
