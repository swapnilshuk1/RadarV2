...
export class ProductionStagedEvaluationService {
  ...
  async evaluate(identity:ProductionStagedIdentity & {context:EvaluationContext},onStage:(stage:string)=>void=()=>{}):Promise<StagedEvaluationRecord>{
    ...
    const staged=await runStagedFrozenDecisionDetailed(frozen,this.model,onStage,{policyVersion:identity.context.policyVersion==='staged-v8'?'staged-v8':identity.context.policyVersion==='staged-v7'?'staged-v7':'staged-v6'});
