...
import type { StagedDecisionResult } from '@/dossier/staged-decision-contract';
...
export class ProductionStagedDossierService {
  ...
  async compose(identity:ProductionStagedIdentity,onStage:(stage:string)=>void=()=>{}) {
    ...
    const existing=await store.get(identity,evaluation.inputFingerprint);
    ...
    const staged=evaluation.evaluation as StagedDecisionResult;
    ...
    try {
      const dossier=await composeStagedDossier(frozen,staged,this.model,onStage);
      await store.save(identity,evaluation.inputFingerprint,dossier);
      return dossier;
    }catch(error){
      await store.recordFailure(identity,evaluation.inputFingerprint,error);
      throw error;
    }
  }
}
