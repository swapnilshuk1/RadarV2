...
export const Route=createFileRoute('/scraped')({
  loader:({location})=>{
    const raw=location.search as {offset?:unknown;tenantId?:unknown;personId?:unknown};
    const deps={offset:Math.max(0,Number(raw.offset)||0),tenantId:typeof raw.tenantId==='string'?raw.tenantId:undefined,personId:typeof raw.personId==='string'?raw.personId:undefined};
...
function ScrapedFeed(){
  ...
  const {offset=0,tenantId,personId}=Route.useSearch() as {offset?:number;tenantId?:string;personId?:string};
  ...
  return <main className="mx-auto max-w-5xl px-4 py-10 sm:px-8">
    ...
    <ul className="divide-y">{rows.map(row=><li key={row.id+':'+row.version} className="flex flex-wrap items-center justify-between gap-4 py-5">
      ...
      {row.state==='READY'?<Link className="rounded border px-3 py-2 text-sm hover:bg-muted" to="/opportunity/$jobHash" params={{jobHash:row.jobHash}} search={scope}>{row.decision} · View dossier</Link>:<span className="rounded border px-3 py-2 text-sm text-muted-foreground">{labels[row.state]}</span>}
    </li>)}</ul>
    ...
    <nav aria-label="Scraped jobs pages" className="mt-8 flex justify-between">
      {offset>0?<Link to="/scraped" search={{...scope,offset:Math.max(0,offset-100)}}>Previous</Link>:<span/>}
      {nextOffset!==null&&<Link to="/scraped" search={{...scope,offset:nextOffset}}>Next</Link>}
    </nav>
  </main>;
}
