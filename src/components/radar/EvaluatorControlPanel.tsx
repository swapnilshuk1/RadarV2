...
export function EvaluatorControlPanel() {
  ...
  return (
    <details className="memo-card mb-space-6" data-testid="evaluator-control-panel">
      ...
      {!snapshot ? (
        <p className="mt-space-3 border-t border-border pt-space-3 text-sm text-muted-foreground">
          Loading evaluator telemetry…
        </p>
      ) : (
        <div className="mt-space-3 space-y-space-4 border-t border-border pt-space-3">
          <div className="flex flex-wrap gap-space-2">
            {snapshot.control.canControl && (
              <>
                ...
                snapshot.control.localDaemonRunning ? (
                  <button
                    ...
                    className="rounded border border-border px-3 py-2 text-xs font-mono font-bold uppercase tracking-wider hover:bg-muted disabled:opacity-50"
                  >
                    Pause
                  </button>
                ) : (
                  <button
                    ...
                    onClick={() =>
                      void act(
                        snapshot.control.desiredState === "PAUSED" ? "resume" : "start",
                      )
                    }
                    className="rounded bg-foreground px-3 py-2 text-xs font-mono font-bold uppercase tracking-wider text-background disabled:opacity-50"
                  >
                ...
                {snapshot.control.desiredState !== "STOPPED" && (
                  <button
                    ...
                    className="rounded border border-red-500/40 px-3 py-2 text-xs font-mono font-bold uppercase tracking-wider text-red-700 hover:bg-red-500/10 dark:text-red-300 disabled:opacity-50"
                  >
                    Stop
                  </button>
                )}
          ...
          <dl className="grid gap-space-2 sm:grid-cols-3 lg:grid-cols-6">
            {[
              ...
              ["Last completion", snapshot.latestCompletedAt ? formatUtc(snapshot.latestCompletedAt) : "—"],
            ].map(([label, value]) => (
          ...
          {snapshot.activeJobs.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground">No evaluation job is currently processing.</p>
          )}
