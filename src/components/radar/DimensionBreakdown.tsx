...
function Group({ label, items }: { label: string; items: DimensionResult[] }) {
  ...
  return (
    <section>
      <p className="mb-3 label-mono text-ink-muted">{label}</p>
          ...
          <div key={d.key} className="grid grid-cols-[180px_minmax(0,1fr)] gap-6 py-4">
            ...
            <div className="space-y-2">
              {d.jdEvidence.evidence[0] ? (
                <blockquote className="border-l-2 border-brass/60 pl-3 font-serif italic text-base leading-snug text-ink">
                  “{d.jdEvidence.evidence[0].quote}”
                  <span className="mt-1 block font-sans not-italic label-mono text-ink-muted">
                    Source · {d.jdEvidence.evidence[0].source}
                  </span>
                </blockquote>
              ) : (
                <p className="font-sans text-sm italic text-ink-muted">No verbatim evidence in the job description. RADAR will not infer.</p>
              )}
              {d.candidateProof && (
                <div className="mt-2 border border-hairline bg-card px-3 py-2">
                  <p className="label-mono text-brass">From your career</p>
