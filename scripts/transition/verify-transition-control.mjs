...
function requireSha(label, value) {
  if (typeof value !== "string" || !/^[0-9a-f]{40}$/i.test(value)) {
    die(`Invalid ${label}: ${String(value)}`);
  }
}

if (!existsSync(statePath)) die("Missing docs/transition/RADAR_TRANSITION_STATE.json");
...
if (batch.scopeRevision !== state.currentBatchScopeRevision) {
  die(`Batch scope revision mismatch: state=${state.currentBatchScopeRevision}, manifest=${batch.scopeRevision}`);
}
if (batch.controlRevision !== state.controlRevision) {
  die(`Control revision mismatch: state=${state.controlRevision}, manifest=${batch.controlRevision}`);
}
if (batch.status !== "READY" && batch.status !== "ACTIVE" && batch.status !== "COMPLETE") {
  die(`Unsupported batch status: ${String(batch.status)}`);
}

requireSha("batchStartCommit", batch.batchStartCommit);
requireSha("gateEntryCommit", state.gateEntryCommit);
...
if (state.activeGate === "GATE_1B") {
  ...
  requiredControlText.push(`**Gate 1B — Batch 02: ${batch.title}.**`);
}
if (state.extractionArchitectureDecision?.status === "OPEN") {
  ...
  requiredControlText.push("**Batch 03 is not authorized by this Batch 02 activation.**");
}
...
function requireExistingRecord(label, path) {
  ...
  if (!existsSync(resolve(root, path))) die(`${label} points to a missing file: ${path}`);
}
...
if (state.activeGate === "GATE_3") requireExistingRecord("Gate 2 exit certification", gateExit.gate2);
...
if (!hardForbidden) die(`No hard gate-path policy exists for active gate ${state.activeGate}`);
...
const gateChanged = new Set();
addLines(gateChanged, git(["diff", "--name-only", `${state.gateEntryCommit}...HEAD`]));
for (const file of worktreeChanged) gateChanged.add(file);

const gateViolations = [];
for (const file of [...gateChanged].sort()) {
  const blockedBy = hardForbidden.find((prefix) => pathMatchesPrefix(file, prefix));
  if (blockedBy) gateViolations.push(`${file} — hard-blocked by ${state.activeGate} boundary (${blockedBy})`);
}
if (gateViolations.length > 0) {
  console.error("TRANSITION CONTROL FAIL: changed files cross the active gate boundary:");
  for (const violation of gateViolations) console.error(`  - ${violation}`);
  process.exit(1);
}

const batchChanged = new Set();
addLines(batchChanged, git(["diff", "--name-only", `${batch.batchStartCommit}...HEAD`]));
for (const file of worktreeChanged) batchChanged.add(file);
...
for (const file of [...batchChanged].sort()) {
  if (!allowedPrefixes.some((prefix) => pathMatchesPrefix(file, prefix))) {
    batchViolations.push(`${file} — outside current batch allowlist`);
  }
}
if (batchViolations.length > 0) {
  console.error("TRANSITION CONTROL FAIL: changed files exceed the current batch scope:");
  for (const violation of batchViolations) console.error(`  - ${violation}`);
  console.error("\nStop before coding further. Amend control/state/batch authorization explicitly rather than bypassing this check.");
  process.exit(1);
}
...
requireSha("ledger enforcementStartCommit", ledger.enforcementStartCommit);
...
const governanceOnlyPrefixes = Array.isArray(ledger.governanceOnlyPathPrefixes)
  ...
  : [];
if (governanceOnlyPrefixes.length === 0) die("Implementation ledger has no governanceOnlyPathPrefixes");
...
for (const ack of acknowledgements) {
  ...
  requireSha("implementation-ledger commit", ack.commit);
  if (ackByCommit.has(ack.commit)) die(`Implementation ledger contains duplicate acknowledgement for ${ack.commit}`);
...
if (unacknowledged.length > 0) {
  console.error("TRANSITION CONTROL FAIL: governed commits exist without implementation-ledger acknowledgement:");
...
if (governedWorktree.length > 0) {
  console.error("TRANSITION CONTROL FAIL: uncommitted governed changes exist and cannot be ledger-acknowledged:");
...
if (branch && branch !== "HEAD" && branch !== "main" && branch !== state.workingBranch) {
  console.warn(`TRANSITION CONTROL WARNING: running on ${branch}; governing working branch is ${state.workingBranch}`);
}
...
console.log(`  scope revision: ${batch.scopeRevision}`);
...
console.log(`  changed files since gate entry: ${gateChanged.size}`);
console.log(`  changed files since batch start: ${batchChanged.size}`);
