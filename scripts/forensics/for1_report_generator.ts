...
function main() {
  console.log('================================================================');
  console.log('FOR-1 — GENERATING EXECUTIVE FORENSIC REPORT');
  console.log('================================================================\n');

  const snapshotDb = new Database(SNAPSHOT_PATH, { readonly: true });
  const reportPath = path.join(FORENSICS_DIR, 'FOR-1-current-turso-forensic-report.md');

  const reportMarkdown = `# FOR-1 — EXECUTIVE FORENSIC REPORT: CURRENT TURSO SNAPSHOT ANALYSIS
...
`;
  ...
  console.log(`Wrote Executive Forensic Report to forensics/FOR-1-current-turso-forensic-report.md\n`);
