...
async function testOppPrefix() {
  ...
  console.log("=== EXACT OPPORTUNITY RECONCILIATION WITH opp_ PREFIX HANDLING ===");
  ...
  if (newCanonicalCandidates.length > 0) {
    console.log("Remaining new canonical candidates count:", newCanonicalCandidates.length);
    console.log("Sample remaining (first 5):", newCanonicalCandidates.slice(0, 5));
  }
}

testOppPrefix().catch(console.error);
