...
describe("RADAR V4 Pipeline Repair - Regression Suite", () => {
  const dummyCandidate = CandidateProjectionBuilderImpl.build({
    name: "Executive Candidate",
    title: "Chief Marketing Officer",
    summary: "Senior GTM and Growth Executive with Enterprise P&L ownership.",
    coreCapabilities: ["GTM Strategy", "Growth Marketing", "Demand Generation", "P&L Management"],
    executiveThemes: ["Commercial Leadership", "Digital Transformation"],
    commercialScope: { value: "ENTERPRISE", source: "RESUME" },
    decisionAuthority: { value: "ENTERPRISE", source: "RESUME" },
    operatingLevel: { value: "EXECUTIVE", source: "RESUME" },
  } as any);
  ...
  it("Test 1: Full JD generates structured dimensions with valid verbatim quotes", async () => {
    const extraction = await extract(mockFullCard);
    ...
    for (const d of validDims) {
      if (d.jdEvidence.evidence && d.jdEvidence.evidence.length > 0) {
        for (const ev of d.jdEvidence.evidence) {
          if (ev.quote) {
            expect(mockFullCard.detail.rawText).toContain(ev.quote);
          }
        }
      }
    }
  ...
  it("Test 4: Full JD supersedes snippet extractions", async () => {
    ...
    const snippetEx = await extract(snippetCard);
    const fullEx = await extract(mockFullCard);
