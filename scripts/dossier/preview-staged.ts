...
const context=option('context'),job=option('job');
if(!context||!job)throw new Error('Explicit --context and --job required');
...
const dossier=dossierSchema.parse(JSON.parse(row.presentation_json));
