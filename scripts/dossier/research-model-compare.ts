...
import { prepareFrozenResearchInput, researchInputFingerprint, runFrozenResearch } from '../../src/dossier/pipeline';
...
const prior=JSON.parse(await readFile(values.evidence,'utf8')) as { evidence:{lineage:SliceInput['sources']}; acquisition: import('../../src/dossier/contracts').AcquisitionAttempt[] };
...
const provider:ContextProvider={id:'frozen-context',async acquire(){return {sources:[],attempts:prior.acquisition};}};
...
for (const version of models) {
  ...
  if (before !== frozen.fingerprint) throw new Error(`Frozen research input changed before ${version}`);
  const research=await runFrozenResearch(frozen,model);
  const after=researchInputFingerprint(frozen);
  if (after !== before) throw new Error(`Frozen research input changed during ${version}`);
  results.push({model:`${model.id}/${model.version}`,frozenInputFingerprint:before,research});
}
