...
import {
  ...
  ACQUISITION_INGRESS_PATH,
  ACQUISITION_INGRESS_RUN_PATH,
  type AcquisitionEnvelope,
} from "../../../src/lib/acquisition/ingress";

export interface OracleIngressScope {
  tenantId: string;
  personId: string;
  searchPlanId?: string | null;
}
export type OracleIngressRunStatus = "enriching" | "failed" | "aborted";
...
function configuredScope(scope?: OracleIngressScope): OracleIngressScope {
  ...
  if (!tenantId || !personId)
    throw new Error(
      "ORACLE_INGRESS_SCOPE_REQUIRED: tenant and person must be supplied by the authenticated local scraper configuration.",
    );
  return { tenantId, personId, searchPlanId };
}

/** Sends bytes, never local artifact paths, to the Oracle durable boundary. */
export async function submitToOracleIngress(
  payload: IngestOpportunityPayload,
  runId: string,
  scope?: OracleIngressScope,
  ...
  if (
    portablePayload.enrichmentDispatch?.detailedCard &&
    typeof portablePayload.enrichmentDispatch.detailedCard === "object"
  ) {
  ...
  const contentHash = computeContentHash({
    title: portablePayload.jobTitle,
    companyName: portablePayload.companyName,
    location: portablePayload.location,
    employmentType: portablePayload.employmentType,
    rawContent: portablePayload.rawContent,
  });
  const submissionId = crypto
    .createHash("sha256")
    .update(
      `${runId}:${effectiveScope.tenantId}:${effectiveScope.personId}:${portablePayload.sourcePortal}:${portablePayload.sourceJobId}:${contentHash}`,
    )
    .digest("hex");
  const envelope: AcquisitionEnvelope = {
    ...
    searchPlanId: effectiveScope.searchPlanId,
    contentHash,
    payload: portablePayload,
  };
  const response = await fetch(new URL(ACQUISITION_INGRESS_PATH, baseUrl).toString(), {
    method: "POST",
  ...
  if (!response.ok || !parsed?.result)
    throw new Error(`ORACLE_INGRESS_REJECTED: ${parsed?.error || response.status}`);
  return parsed.result;
}

async function updateOracleIngressRunStatus(
  runId: string,
  status: OracleIngressRunStatus,
  scope?: OracleIngressScope,
): Promise<void> {
  const baseUrl = process.env.RADAR_ACQUISITION_INGRESS_URL;
  const secret = process.env.RADAR_ACQUISITION_INGRESS_SECRET;
  ...
  if (!response.ok)
    throw new Error(`ORACLE_INGRESS_RUN_TERMINALIZATION_REJECTED: ${response.status}`);
}

/** The browser-owning producer has finished submission; Oracle owns downstream completion. */
export async function markOracleIngressRunEnriching(
  runId: string,
  scope?: OracleIngressScope,
): Promise<void> {
  await updateOracleIngressRunStatus(runId, "enriching", scope);
}

export async function terminalizeOracleIngressRun(
  runId: string,
  status: "failed" | "aborted",
  scope?: OracleIngressScope,
): Promise<void> {
  await updateOracleIngressRunStatus(runId, status, scope);
}
