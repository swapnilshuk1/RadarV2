...
describe("Stage 2B: Canonical Identity & Candidate Projection Unification", () => {
  ...
  it("verifies legacy identity 'swapnil-shukla' is completely purged from people table", async () => {
    const legacyPerson = await db.one("SELECT id FROM people WHERE id = ?", [LEGACY_ID]);
    expect(legacyPerson).toBeNull();
  });

  it("verifies zero orphaned records reference the legacy identity in any table", async () => {
    const tables = await db.many("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'");
    for (const t of tables) {
      const pragma = await db.many(`PRAGMA table_info("${t.name}")`);
      for (const col of pragma) {
        if (["person_id", "user_id", "candidate_id", "owner_id", "id"].includes(col.name)) {
          const check = await db.many(
            `SELECT count(*) as c FROM "${t.name}" WHERE "${col.name}" = ?`,
            [LEGACY_ID]
          );
