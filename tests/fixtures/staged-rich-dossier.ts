...
import {
  ...
  reviewPassages,
} from "../../src/dossier/factual-review-integrity";
      ...
      requirements: [
        {
          ...
          roleClaimIds: ["JD-1-1"],
          reasoning: "Growth is core to the mandate.",
        },
      ],
      operatingConditions: [],
    ...
    requirements: [
      {
        ...
        screeningReasoning: "The source describes role performance, not an entry gate.",
        status: "NOT_EVIDENCED",
        candidateClaimIds: [],
        unsupportedAspects: ["direct growth precedent"],
        mappingReasoning:
          "The supplied candidate evidence does not directly establish this requirement.",
      },
    ],
    resolutions: [
      {
        ...
        claimIds: [],
        methods: ["ask"],
        question: "Who owns the growth mandate?",
...
export function dossier(
  trace: JsonValue = stagedEvaluation.trace as unknown as JsonValue,
): Dossier {
  const p = (text: string): Passage => ({
    text,
    kind: "ADVICE",
    state: "INFERRED",
    confidence: 0.8,
    sourcePlane: "JD",
    evidenceRefs: ["JD-1-1"],
    reasoning: "Derived from the supplied growth mandate.",
  });
  const list = (text: string) => [p(text)];
  const result: Dossier = {
    opportunity: { id: "job", company: "Company", title: "Head of Growth" },
    candidate: { name: "Candidate" },
    executiveThesis: { ...p("Assess the growth mandate."), kind: "CONCLUSION" },
    opportunityValue: list("Commercial value"),
    mandate: {
      priorities: list("Establish operating ownership."),
      outcomes: list("Growth outcomes"),
    },
    candidateFit: [
      {
        label: "Growth precedent",
        requirementIds: ["REQ-001"],
        assessment: p("Validate the growth precedent."),
      },
    ],
    decisionConditions: [
      {
        requirementIds: ["REQ-001"],
        resolutionFields: [],
        question: { ...p("Who owns growth?"), kind: "QUESTION" },
        consequence: { ...p("Authority determines the career value."), kind: "CONCLUSION" },
      },
    ],
    approach: {
      nextSteps: list("Explore the scope."),
      opening: p("I would like to understand your growth mandate."),
      resumeNarrative: [],
      linkedinStrategy: [],
      screening: [],
      interview: [],
    ...
    verdict: {
      verdict: "PURSUE",
      screeningViability: "PLAUSIBLE",
      rationale: "Growth mandate",
      claimIds: ["JD-1-1"],
      requirements: [
        {
          requirement: "Growth",
          mandatory: true,
          decisionRole: "CORE_CAPABILITY",
          status: "NOT_EVIDENCED",
          roleClaimIds: ["JD-1-1"],
          candidateClaimIds: [],
          reasoning: "Confirm proof",
        },
      ],
    },
    narrativePlan: {
      ...
      careerMove: "Growth",
      authorityShape: "Function",
      fitShape: "Transferable",
      evidenceShape: "Mandate",
      decisionTension: "Scope",
      companyTrajectory: "Unknown",
      argument: "Clarify authority",
      emphasis: ["Scope"],
      ...
      memoPoints: [
        {
          id: "fit",
          section: "candidateFit",
          point: "Validate growth precedent",
          claimIds: ["JD-1-1"],
          requirementIds: ["REQ-001"],
          resolutionFields: [],
        },
        {
          ...
          point: "Resolve growth authority",
          claimIds: ["JD-1-1"],
          requirementIds: ["REQ-001"],
          resolutionFields: [],
        },
    ...
    resolutions: stagedEvaluation.trace.resolutions,
    candidateConflicts: [],
    evidence: {
      roleClaims: [
        {
    ...
    generation: {
      ...
      factualReviewer: { model: "fixture-review/1", policyVersion: FACTUAL_REVIEW_POLICY_VERSION },
    },
    acquisition: [],
    canonicalDecisionTrace: trace,
  ...
  result.generation.factualReviews = Object.keys(compositionSchema.shape).map((section) => {
    ...
    return {
      ...
      evidenceFingerprint: reviewEvidenceFingerprint(
        ...
        result.candidateConflicts,
      ),
      inputFingerprint: "input",
      reviewer: "fixture-review/1",
      policyVersion: FACTUAL_REVIEW_POLICY_VERSION,
      passageIds: reviewPassages(value).map((p) => p.passageId),
      accepted: true,
      coveredPointIds: result.narrativePlan
        .memoPoints!.filter((p) => p.section === section)
        .map((p) => p.id),
      planFingerprint: reviewFingerprint(
        result.narrativePlan.memoPoints!.filter((p) => p.section === section),
      ),
    };
  });
  return result;
