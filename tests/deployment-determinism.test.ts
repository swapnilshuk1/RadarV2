...
describe("RADAR Stage 2C — Deployment Determinism & Production Invariants", () => {
  ...
  it("4. deploy.sh does not package radar.sqlite or live-scraped.json", () => {
    const deployShPath = path.resolve(process.cwd(), "deploy.sh");
    const content = fs.readFileSync(deployShPath, "utf-8");

    // Must NOT contain direct inclusion of radar.sqlite or live-scraped.json in tar args
    expect(content).not.toMatch(/^\s*radar\.sqlite\s*\\/m);
    expect(content).not.toMatch(/^\s*src\/data\/live-scraped\.json\s*\\/m);

    // Must explicitly exclude them
    expect(content).toContain("--exclude='*.sqlite*'");
    expect(content).toContain("--exclude='live-scraped.json'");
    expect(content).toContain("--exclude='.env*'");
  });

  it("5. deploy.sh uses GNU tar compliant flag ordering (excludes precede targets)", () => {
    const deployShPath = path.resolve(process.cwd(), "deploy.sh");
    const content = fs.readFileSync(deployShPath, "utf-8");

    const excludeIdx = content.indexOf("--exclude=");
    const czfIdx = content.indexOf("-czf");
    const outputIdx = content.indexOf(".output/");

    expect(excludeIdx).toBeGreaterThan(0);
    expect(czfIdx).toBeGreaterThan(excludeIdx);
    expect(outputIdx).toBeGreaterThan(czfIdx);
  });
  ...
  it("8. OpportunityService.listForUser accepts opportunities from repositories exclusively", () => {
    ...
    expect(serviceContent).toContain("repos.opportunities.listOpportunitySources()");
