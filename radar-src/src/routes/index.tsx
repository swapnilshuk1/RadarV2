...
import { triggerScrapeFn } from "../lib/intelligence/scrape-server";
  ...
  const runSearch = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const now = new Date();
        setLastScanAt(now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }));
      } else {
        ...
        <div className="mx-auto flex max-w-4xl items-center justify-between gap-6 px-8 py-5">
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <span
                aria-hidden
                className={`inline-block h-1.5 w-1.5 rounded-full bg-parchment ${scanning ? "animate-pulse" : ""}`}
              />
