...
export const linkedinHandler: PortalHandler = {
  ...
  async ensureSession(ctx) {
    ...
    try {
      ...
      await sleep(2000);
      const loggedIn = await page.locator('input[placeholder*="Search" i]').first().count().catch(() => 0);
      if (!loggedIn) {
        ...
        while (Date.now() < deadline) {
          const ok = await page.locator('input[placeholder*="Search" i]').first().count().catch(() => 0);
          if (ok) return "ready";
