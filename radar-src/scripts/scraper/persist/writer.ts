...
import fs from "fs";
import type { ExtractionResult, JobSnapshot } from "../types";
...
export function readSnapshotIfFresh(cardHash: string, maxAgeHours: number): JobSnapshot | null {
  const p = snapshotPath(cardHash);
  if (!fs.existsSync(p)) return null;
  if (fileAgeHours(p) > maxAgeHours) return null;
  return readJsonSafe<JobSnapshot>(p);
}

export function writeSnapshot(s: JobSnapshot): string {
